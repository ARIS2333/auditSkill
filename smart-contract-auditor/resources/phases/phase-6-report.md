...
**Inputs:**
- `audit-output/phase-5-findings/` (all finding folders with PoCs and write-ups)
- `audit-output/phase-2-docs/codebase-overview.md` (for cross-checking claims)
- `audit-output/phase-4-analysis/triage.md` (for prior-audit-overlap verdicts)
- Platform template: `resources/templates/code4rena.md` or `resources/templates/sherlock.md`

**Outputs:** Write to `audit-output/phase-6-report/`:
...
5. **Cross-check known issues** — for each finding tagged `prior-audit-overlap` in `triage.md`, verify the Phase 4 verdict: true duplicates are excluded; findings with higher demonstrated severity or a distinct attack path remain with an explicit note referencing the prior finding.
